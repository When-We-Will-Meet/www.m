...
var pwImg1 = document.querySelector('#pwd_img1');
...
function comparePw() {
    $('#pswd2').keyup(function(){
        ...
        if($('#pwd').val() != $('#pswd2').val()){
            error[2].innerHTML = "비밀번호가 일치하지 않습니다.";
            error[2].style.display = "block";
            $('#errorNotice').html('비밀번호 일치하지 않음<br><br>');
            $('#errorNotice').attr('color', '#f82a2aa3');
        } else{
            error[2].innerHTML = "";
            error[2].style.display = "block";
            $('#errorNotice').html('비밀번호 일치함<br><br>');
            $('#errorkNotice').attr('color', '#03c75a');
        }
